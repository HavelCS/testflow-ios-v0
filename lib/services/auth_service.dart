...
class AuthService {
  ...
  // Get liveness session results (must be called first when session completes)
  static Future<Map<String, dynamic>> getLivenessSessionResults(
    String sessionId,
  ) async {
    try {
      print('📥 Getting liveness session results for session: $sessionId');
      final headers = await getAuthHeaders();
      
      final response = await http.get(
        Uri.parse('$_baseUrl$_facesEndpoint/liveness/results/$sessionId'),
        headers: headers,
      );

      print('📡 GET Results response status: ${response.statusCode}');
      print('📡 GET Results response body: ${response.body}');
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        print('✅ Successfully retrieved liveness session results');
        return {
          'success': true,
          'confidence': data['confidence'],
          'isLive': data['isLive'],
          ...
          'message': 'Liveness results retrieved successfully',
        };
      } else if (response.statusCode == 401) {
        await clearAuthData();
        return {
        ...
        try {
          final error = json.decode(response.body);
          errorMessage = error['detail'] ?? error['message'] ?? 'Failed to get liveness results';
        } catch (e) {
          errorMessage = 'Server error (${response.statusCode}): ${response.body}';
        }
        print('❌ getLivenessSessionResults error: $errorMessage');
        return {
          'success': false,
      ...
      print('❌ getLivenessSessionResults exception: $e');
      return {
        'success': false,
  ...
  static Future<Map<String, dynamic>> processLivenessResults(
    ...
    try {
      print('💾 Calling processLivenessResults API for session: $sessionId');
      final headers = await getAuthHeaders();
      print('🔑 Headers prepared: ${headers.keys}');
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        print('✅ Successfully processed liveness results');
        return {
          ...
          'message': data['message'] ?? 'Processing initiated',
        };
      } else if (response.statusCode == 401) {
        await clearAuthData();
        return {
        ...
        try {
          final error = json.decode(response.body);
          errorMessage = error['detail'] ?? error['message'] ?? 'Failed to process results';
        } catch (e) {
          errorMessage = 'Server error (${response.statusCode}): ${response.body}';
        }
        print('❌ processLivenessResults error: $errorMessage');
        return {
          'success': false,
          'message': errorMessage,
        };
      ...
      print('❌ processLivenessResults exception: $e');
      return {
        'success': false,
        'message': 'Network error: $e',
      };
  ...
  static Future<Map<String, dynamic>> processLivenessResultsWithPolling(
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        if (userStatusResult['success'] == true &&
            userStatusResult['status'] == 'verified') {
          print(
            '✅ User status changed to verified in database, stopping face scan polling',
          );
          return {
            'success': true,
            'isNewFace': true,
